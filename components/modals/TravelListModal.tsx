...
const TravelListModal: React.FC<TravelListModalProps> = ({ 
  ...
  // Filter and Sort travels for this group from the global context state
  const travels = useMemo(() => {
    if (!group) return [];
    return allTravels
      .filter(t => t.groupId === group.id)
      .sort((a, b) => {
         // Try to sort by Date field first, then by Name (if it starts with a date), then ID
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await services.travels.delete(itemToDelete);
      // Context subscription will automatically update the UI
                                  ...
                                  <div className="text-sm text-gray-500 mt-1 space-y-1">
                                      <div className="flex flex-wrap items-center gap-4">
                                          <div className="flex items-center gap-1">
                                            <span className="text-xs uppercase font-bold text-sage-400">Date:</span>
                                            <span>{travel.date || 'N/A'}</span>
                                          </div>
                                          <div className="flex items-center gap-1">
